# PHP Quickmux: High-Performance Router

Quickmux is an ultra-low latency request dispatcher and routing library designed for high-throughput microservices. It rejects the overhead of full-stack frameworks in favor of a direct, O(1) matching algorithm that maps HTTP requests to strict PSR-15 request handlers with minimal memory allocation.

The implementation prioritizes execution speed and predictability. It creates a thin, transparent layer between the web server and the application logic, ensuring that routing logic never becomes a bottleneck in the request lifecycle. It is fully compliant with PSR-7, PSR-15, and PSR-17 standards.
